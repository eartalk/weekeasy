...
- 以后新增或修改数据库表、字段时，必须为表和字段添加清晰的中文注释；注释必须通过迁移 SQL（如 `COMMENT ON TABLE`、`COMMENT ON COLUMN`）同步到实际数据库，不能只写在 Prisma Schema 或业务代码中。
...
Git 提交信息的描述必须使用中文，并保持 Conventional Commits 格式，例如：`feat(api): 新增游客档案接口`。
